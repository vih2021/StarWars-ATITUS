...
export default function Login(props) {
  ...
  let acessos = Number(localStorage.getItem("contagens"))
  const [email, setEmail] = useState("")
  const [password, setPassword] = useState("")

  const somar = () =>{
    acessos = acessos + 1
    localStorage.setItem("contagens", acessos)
    console.log(localStorage.getItem('contagens'))
  }
    ...
    <Grid container style={{ padding: 10 }}>
      <Grid item xs={12}>
        <div style={{ textAlign: "center" }}>
          <h1 className="LoginText">STAR WARS</h1>
